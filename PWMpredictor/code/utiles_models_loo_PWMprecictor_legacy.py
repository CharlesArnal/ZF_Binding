...
def pipeline_func(c_rc_df, deployment_df, b1h_model, folder_address, lr, epochs, res_num, start1):

    #c_rc_df = c_rc_df[c_rc_df['groups'] != 0]
    label_mat = get_label_mat(c_rc_df)
    data_input_model = create_input_model(c_rc_df, res_num)

    label_deployment = get_label_mat(deployment_df)
    data_deployment_model = create_input_model(deployment_df, res_num)
    
    

    x_train, x_deployment = data_input_model, data_deployment_model
    y_train, _ = np.asarray(label_mat), np.asarray(label_deployment)
    model = b1h_model
    opt = Adam(learning_rate=lr)
    model.compile(loss='categorical_crossentropy', optimizer=opt)
    history = model.fit(x_train, y_train, epochs=epochs, batch_size=10, verbose=2)
    model.summary()
    model.save(folder_address + '/models/' + 'transfer_model_all.h5')
    with open(folder_address + '/history/' + 'transfer_history_all', 'wb') as hist_file:
        pickle.dump(history.history, hist_file)

    # model deployment
    y_predicted = model.predict(x_deployment)
    np.save(folder_address + '/predictions/' + 'pred_deployment', y_predicted)
    clear_session()

def pipeline_func_legacy(c_rc_df, zf_df, b1h_model, folder_address, lr, epochs, res_num, start1):
    miss_index = []
    n_split = c_rc_df['groups'].unique().shape[0]
    y_test_list = []
    y_predicted_list = []

    for i in  {0}:#range(n_split):
